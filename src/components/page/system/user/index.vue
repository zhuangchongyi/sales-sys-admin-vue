<template>
    <div class="container">
            ...
            <el-col :span="20" :xs="24">
                ...
                <el-table v-loading="loading" :data="dataList">
                    ...
                    <el-table-column label="操作" align="center" width="300" fixed="right" class-name="small-padding fixed-width">
                        <template slot-scope="scope">
                            <el-button size="mini" type="text" icon="el-icon-edit" v-if="scope.row.userId != 1" v-hasPermi="['system:user:role']" @click="handleRole(scope.row)">分配角色</el-button>
                            <el-button
                                size="mini"
                                type="text"
                                ...
                                @click="handleLookMenu(scope.row)"
                                >查看权限</el-button
                            >
                            <el-button
                                size="mini"
        ...
        <!-- 查看菜单权限对话框 -->
        <el-dialog :title="title" :visible.sync="menuOpen" width="600px" append-to-body>
...
export default {
    ...
    methods: {
        ...
        handleResetPassword(row) {
            let that = this;
            this.$confirm('确认是否重置登录账号为 <strong><i>' + row.username + '</i></strong> 的密码?', '提示', {
                ...
                .then(() => {
                    resetPassword(row.userId).then(res => {
                        if (res.success) {
                            that.$alert('<div style="text-align: center;font-size: 16px;"><strong>' + res.data + '</strong></div>', '重置成功', {
                                dangerouslyUseHTMLString: true
