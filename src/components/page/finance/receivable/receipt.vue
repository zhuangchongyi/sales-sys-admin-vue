<template>
    <div class="container">
        ...
        <el-table v-loading="loading" :data="receivablelistData" ref="receivablelistData">
            ...
            <el-table-column label="操作" align="center" width="200" fixed="right">
                <template slot-scope="scope">
                    <el-button size="mini" type="text" icon="el-icon-info" @click="handlePreview(scope.row)">应收明细</el-button>
        ...
        <!-- 订单弹窗 -->
        <el-dialog title="签回产品信息" :visible.sync="orderOpen" width="1000px" append-to-body v-dialogDrag>
            <el-table v-loading="loading" :data="orderListData" show-summary :summary-method="getSummaries">
...
<script>
import { getReceivable, listReceivableSub } from '@/api/finance/receivable.js';
...
export default {
    ...
    watch: {
        $route(to, form) {
            if (to.path === '/page/finance/receivable/receipt' && this.clienteleForm.receivableId !== this.$route.query.rid) {
                this.getListData();
            }
        }
    },
    methods: {
        validateReceivePrice(rule, value, callback) {
            if (parseFloat(value) > parseFloat(this.totalPrice)) {
                callback(new Error('收款金额超过应收金额'));
            } else {
                callback();
            }
        },
        getListData() {
            ...
            if (id) {
                getReceivable(id).then(res => {
                    if (res.data) {
                        this.receivablelistData.splice(1, 1, res.data);
                        this.totalPrice = res.data.totalPrice;
                        this.$set(this.clienteleForm, 'clienteleId', res.data.clienteleId);
                        this.$set(this.clienteleForm, 'clienteleNum', res.data.clienteleNum);
                        this.$set(this.clienteleForm, 'clienteleName', res.data.clienteleName);
                ...
                if (valid) {
                    ...
                    addReceipt(this.clienteleForm).then(res => {
                        ...
                        if (res.success) {
                            ...
                            this.clienteleForm.receiptNum = res.data;
                        } else {
        ...
        handlePreview(row) {
            this.orderOpen = true;
            listReceivableSub({ receivableId: row.receivableId }).then(res => {
        ...
        getSummaries(param) {
            ...
            const sums = [];
            const propertys = ['price', 'number', 'signNum', 'shipmentNum', 'totalPrice'];
