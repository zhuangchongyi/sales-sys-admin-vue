<template>
    <div class="container">
        <div>
            <el-button type="primary" icon="el-icon-check" @click="submitAddForm" :loading="btnLoading">确认生成</el-button>
            <el-button icon="el-icon-refresh" @click="getQuotation">还原</el-button>
        </div>
        ...
        <el-table v-loading="loading" :data="materielListData">
            <el-table-column label="产品编码" align="center" :show-overflow-tooltip="true" prop="materielNum"> </el-table-column>
...
import { listQuotationSub, addQuotationSub, updateQuotationSub, deleteQuotationSub, generateOrder } from '@/api/sales/quotation.js';
...
export default {
    ...
    created() {
        ...
        this.quotationId = this.$route.query.qId;
        this.getQuotation();
    },
    watch: {
        $route(to, form) {
            if (to.path === '/page/sales/quotation/order' && this.quotationId !== this.$route.query.qId) {
                this.quotationId = this.$route.query.qId;
                this.getQuotation();
            }
        }
    ...
    methods: {
        ...
        submitAddForm() {
            this.$refs['clienteleForm'].validate(valid => {
                if (valid) {
                    let quotationTime = new Date(this.parseTime(this.clienteleForm.quotationTime, '{y}-{m}-{d}'));
                    ...
                    generateOrder(data)
                        .then(res => {
                            ...
                            if (res.success) {
                                this.msgSuccess('订单生成成功，' + res.data);
                            } else {
