<template>
    <div class="container">
        ...
        <el-form :model="clienteleForm" ref="clienteleForm" :rules="rules" label-position="right" label-width="100px" :inline="true">
            <el-row>
                <el-col :span="4">
                    <el-form-item label="应付单号" prop="payableNum">
                        <el-input v-model="clienteleForm.payableNum" size="small" style="width: 150px;" readonly />
                    </el-form-item>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                    <el-form-item label="退货单号" prop="returnsNum">
                        <el-input v-model="clienteleForm.returnsNum" size="small" style="width: 150px;" readonly />
                    </el-form-item>
                </el-col>
                <el-col :span="4">
                    <el-form-item label="退货日期" prop="returnsTime">
                        <el-date-picker v-model="clienteleForm.returnsTime" size="small" style="width: 150px;" readonly />
                    </el-form-item>
                </el-col>
                <el-col :span="4">
        ...
        <!--  添加客户窗口 -->
        <el-dialog title="客戶退货单" :visible.sync="open" width="60%" append-to-body>
            <el-form :model="clienteleQueryParams" ref="clienteleQueryParams" :inline="true">
                <el-form-item prop="returnsNum">
                    <el-input v-model="clienteleQueryParams.returnsNum" placeholder="退货单号" clearable size="small" style="width: 150px" @keyup.enter.native="handleQuery" />
                </el-form-item>
                <el-form-item prop="orderNum">
                    <el-input v-model="clienteleQueryParams.returnsNum" placeholder="订单号" clearable size="small" style="width: 150px" @keyup.enter.native="handleQuery" />
                </el-form-item>
            ...
            <el-table :data="clienteleListData" highlight-current-row @row-dblclick="getCurrentRow">
                ...
                <el-table-column label="退货单号" align="center" prop="returnsNum" :show-overflow-tooltip="true" width="180px" />
                <el-table-column label="退货日期" align="center" prop="returnsTime" :show-overflow-tooltip="true"> </el-table-column>
                <el-table-column label="订单号" align="center" prop="orderNum" :show-overflow-tooltip="true" width="180px" />
                <el-table-column label="订单日期" align="center" prop="orderTime" :show-overflow-tooltip="true"> </el-table-column>
...
<script>
import { auditPayable, getPayable } from '@/api/finance/payable.js';
...
export default {
    data() {
        return {
            ...
            rules: {
                ...
                returnsNum: [{ required: true, message: '退货单号不能为空', trigger: 'blur' }],
    ...
    watch: {
        $route(to, form) {
            if (to.path === '/page/finance/payable/preview' && this.clienteleForm.payableId !== this.$route.query.rid) {
    ...
    methods: {
        submitAuditForm() {
            ...
            let data = {
                payableId: this.clienteleForm.payableId,
                status: '3'
            };
            this.btnLoading = true;
            auditPayable(data).then(res => {
        ...
        getCurrentRow(row) {
            ...
            this.loading = true;
            financeSignbackSub({ returnsId: row.returnsId }).then(res => {
                this.loading = false;
        ...
        submitAddForm() {
            this.$refs.clienteleForm.validate(valid => {
                if (valid) {
                    ...
                    addPayable(data).then(res => {
                        if (res.success) {
                            this.msgSuccess('保存成功');
                            this.$set(this.clienteleForm, 'payableNum', res.data);
                        } else {
