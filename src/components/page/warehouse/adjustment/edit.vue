<template>
    <div class="container">
            ...
            <el-row>
                ...
                <el-col :span="4">
                    <el-form-item label="调整时间" prop="adjustmentTime">
                        <el-date-picker v-model="warehouseForm.adjustmentTime" placeholder="请选择日期" size="small" value-format="yyyy-MM-dd" format="yyyy-MM-dd" style="width: 150px" />
                    </el-form-item>
                </el-col>
                <el-col :span="4">
                    <el-form-item label="调整负责人" prop="personnelName">
        ...
        <el-table v-loading="loading" :data="materielListData">
            ...
            <el-table-column prop="adjustNumber" label="现存总数量" align="center"> </el-table-column>
            <el-table-column prop="adjustTotalPrice" label="现存总金额" align="center"></el-table-column> -->
            <el-table-column prop="adjustPrice" label="调整单价" align="center"> </el-table-column>
            <el-table-column prop="number" label="调整数量" align="center">
                <template slot-scope="scope">
                    <el-input size="mini" oninput="value=value.replace(/^0|[^\d]/g,'')" maxLength="9" @input="calculateTotalPrice(scope.row)" v-model="scope.row.number" />
                </template>
            </el-table-column>
            <el-table-column prop="totalPrice" label="调整金额" align="center">
                <template slot-scope="scope">
                    <el-input
                        size="mini"
            ...
            <el-table-column prop="remark" label="调整原因" width="300" align="center">
                <template slot-scope="scope">
...
export default {
    ...
    watch: {
        $router(to, form) {
            if (to.path === '/page/warehouse/adjustment/edit' && this.warehouseForm.adjustmentId !== to.query.id) {
                this.warehouseForm.adjustmentId = to.query.id;
                this.getAdjustmentData();
            }
        }
    },
    created() {
        this.warehouseForm.adjustmentId = this.$route.query.id;
        this.getAdjustmentData();
    },
    methods: {
        getAdjustmentData() {
        ...
        getPersonnelName() {
            ...
            this.warehouseForm.adjustmentTime = this.parseTime(new Date());
        },
        // ===================仓库============
        handleAddWarehouse() {
            this.open = true;
        ...
        calculateAll() {
            this.materielListData.forEach(item => {
                item.adjustTotalPrice = item.totalPrice;
                item.adjustNumber = item.number;
                item.adjustPrice = item.price;
                item.adjustNum = item.adjustNum || 0;
            });
        },
        submitMaterielForm() {
        ...
        submitForm() {
            this.$refs['warehouseForm'].validate(valid => {
                if (valid) {
                    ...
                    let data = {
                        adjustment: JSON.stringify(this.warehouseForm),
