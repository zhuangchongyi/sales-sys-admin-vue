<template>
    <div class="container">
        <el-form :model="queryParams" ref="queryParams" :inline="true">
            <el-form-item prop="payableNum">
                <el-input v-model="queryParams.payableNum" placeholder="应付单号" clearable size="small" style="width: 200px" @keyup.enter.native="handleQuery" />
            </el-form-item>
        ...
        <div class="handle-box">
            <el-button type="primary" size="small" icon="el-icon-plus" class="handle-del mr10" @click="handleAdd" v-hasPermi="['finance:payable:add']">新增</el-button>
            <el-dropdown trigger="click" style="margin:0 10px;" v-hasPermi="['finance:payable:submit']">
            ...
            <el-button type="primary" size="small" icon="el-icon-finished" class="handle-del mr10" :disabled="single" @click="handleAudit" v-hasPermi="['finance:payable:audit']">审核</el-button>
        ...
        <el-table v-loading="loading" :data="listData" ref="listData" @row-click="selectionRowClick" highlight-current-row @row-dblclick="handlePreview" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="应付单号" prop="payableNum" align="center" :show-overflow-tooltip="true" width="180" />
            ...
            <el-table-column label="应付金额" prop="totalPrice" :show-overflow-tooltip="true" align="center" />
            ...
            <el-table-column label="操作" width="180" fixed="right" align="center">
                <template slot-scope="scope">
                    <el-button type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['finance:payable:edit']">修改</el-button>
                    <el-button type="text" icon="el-icon-delete" style="color:#fd5656" @click="handleDelete(scope.row)" v-hasPermi="['finance:payable:delete']">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
import { listPayable, deletePayable, submitPayable } from '@/api/finance/payable';
export default {
    data() {
        return {
            loading: false,
    ...
    methods: {
        ...
        handleAdd(row) {
            this.$router.push({ path: '/page/finance/payable/add' });
        },
        handleUpdate(row) {
            if (row.sourceType == '0') {
                this.msgError('不允许操作');
            ...
            this.$router.push({ path: '/page/finance/payable/edit', query: { rid: row.payableId } });
        },
        handlePreview(row) {
            this.$router.push({ path: '/page/finance/payable/preview', query: { rid: row.payableId } });
        },
        handleSubmit() {
            ...
            submitPayable('1', this.ids).then(res => {
                if (res.success) {
        ...
        handleNoSubmit() {
            ...
            submitPayable('2', this.ids).then(res => {
                if (res.success) {
                    this.msgSuccess('提交成功');
                    this.handleQuery();
                } else {
        ...
        handleAudit() {
            ...
            this.$router.push({ path: '/page/finance/payable/preview', query: { rid: this.selection.payableId, isAudit: true } });
        },
        handleCollection(row) {
            if (this.selection.status != '3') {
                this.msgError(this.selection.payableNum + '请先审核');
                return;
            }
            this.$router.push({ path: '/page/finance/payable/receipt', query: { rid: this.selection.payableId } });
        },
        handleDelete(row) {
            console.log(row);
            if (row.sourceType == '0') {
            ...
            this.$confirm('请确认是否删除？', '警告', {
                ...
                deletePayable(row.payableId).then(res => {
                    if (res.success) {
        ...
        verifyStatus(status, msg) {
            return this.checkAuditStatus(this.multipleSelection, 'payableNum', status, msg);
        }
    }
};
</script>
