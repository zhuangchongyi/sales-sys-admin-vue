...
<script>
import { updateReceipt, auditReceipt, getReceipt } from '@/api/finance/receipt.js';
...
export default {
    ...
    watch: {
        $route(to, form) {
            if (to.path === '/page/finance/receipt/preview' && this.clienteleForm.receiptId !== this.$route.query.rid) {
                this.isAudit = Boolean(this.$route.query.isAudit || false);
                this.getReceiptData();
            }
        }
    ...
    methods: {
        submitAuditForm() {
            ...
            let data = {
                receiptId: this.clienteleForm.receiptId,
                status: '3'
            };
        ...
        submitAddForm() {
            this.$refs.clienteleForm.validate(valid => {
                if (valid) {
                    updateReceipt(this.clienteleForm).then(res => {
                        if (res.success) {
                            this.msgSuccess('保存超过');
                            this.$set(this.clienteleQueryParams, 'receiptNum', res.data);
                        } else {
                            this.msgError(res.message);
                        }
                    });
