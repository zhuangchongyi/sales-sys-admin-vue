<template>
    <div class="container">
        ...
        <el-table v-loading="loading" :data="materielListData">
            ...
            <el-table-column prop="hasSignNum" label="累计签收数" align="center"> </el-table-column>
        </el-table>
        ...
        <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
            <el-form :model="supplierQueryParams" ref="supplierQueryParams" :inline="true">
                <el-form-item label="供应商类别" prop="categoryId">
                    <treeselect v-model="supplierQueryParams.categoryId" :options="supplierTypeOptions" :disable-branch-nodes="true" :show-count="true" style="width: 160px" placeholder="请选择类别" />
                </el-form-item>
            ...
            <el-table v-loading="loading" :data="supplierListData" highlight-current-row @row-dblclick="handledbClick">
                ...
                <el-table-column label="供应商编码" align="center" prop="supplierNum" />
                <el-table-column label="供应商名称" align="center" prop="supplierName" :show-overflow-tooltip="true" />
                <el-table-column label="供应商类别" align="center" prop="category.categoryName" :show-overflow-tooltip="true" />
            </el-table>
...
<script>
import { addPurchaseOrder, getPurchaseOrder, auditPurchaseOrder, listPurchaseOrderSub } from '@/api/purchase/order.js';
...
export default {
    ...
    watch: {
        $route(to, form) {
            if (to.path === '/page/purchase/order/preview' && this.orderId !== to.query.id) {
    ...
    methods: {
        submitAuditForm() {
            ...
            let data = {
                order: { orderId: this.orderId, status: '3' }
            };
            this.btnLoading = true;
            auditPurchaseOrder(data).then(res => {
        ...
        cancelAuditForm() {
            ...
            let data = {
                order: { orderId: this.orderId, status: '4' }
            };
            this.btnLoading = true;
            auditPurchaseOrder(data).then(res => {
                this.btnLoading = false;
        ...
        submitClienteleForm() {
            this.supplierForm = this.supplierSelected;
            this.open = false;
        ...
        submitAddForm() {
            ...
            this.$refs['supplierForm'].validate(valid => {
                if (valid) {
                    ...
                    let data = {
                        supplier: JSON.stringify(this.supplierForm),
        ...
        submitFormPersonnel(row) {
            ...
            this.supplierForm.personnelId = this.personnelSelection.userId;
            this.supplierForm.personnelName = this.personnelSelection.nickname;
