<template>
  <div class="container">
      ...
      <el-col :span="20"
        ...
        <el-table v-loading="loading"
          ...
          <el-table-column prop="outTorque"
                           ...
                           :show-overflow-tooltip="true"
                           width="100"></el-table-column>
          <el-table-column prop="units.unitsNum"
                           label="单位编码"
                           align="center"
                           :show-overflow-tooltip="true"
                           width="100"></el-table-column>
          <el-table-column prop="units.unitsName"
                           label="单位名称"
                           align="center"
                           :show-overflow-tooltip="true"
                           width="100"></el-table-column>
          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              <el-button size="mini"
                         type="text"
                         icon="el-icon-edit"
                         v-hasPermi="['basis:materiel:edit']"
                         @click="handleUpdate(scope.row)">修改</el-button>
              <el-button size="mini"
                         type="text"
                         icon="el-icon-lx-upload"
                         style="color:#e6a23c"
                         v-hasPermi="['basis:materiel:upload']"
                         @click="handleUpload(scope.row)">图纸</el-button>
              <el-button size="mini"
                         type="text"
                         icon="el-icon-delete"
                         style="color:#fd5656"
                         v-hasPermi="['basis:materiel:delete']"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="所需扭矩"
                          prop="needTorque">
              <el-input v-model="form.needTorque"
                        placeholder="请输入所需扭矩" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="输出扭矩"
                          prop="outTorque">
              <el-input v-model="form.outTorque"
                        placeholder="请输入输出扭矩" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="单位编码"
                          prop="unitsNum">
              <el-input v-model="form.units.unitsNum"
                        @focus="unitsFocus"
                        ref="unitsBlur"
          ...
          <el-col :span="12">
            <el-form-item label="单位名称"
                          prop="unitsName">
              <el-input v-model="form.units.unitsName"
                        :disabled="true" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    <!-- 基本单位弹窗 -->
    <el-dialog :title="title"
               :visible.sync="unitsOpen"
               width="550px"
               append-to-body>
      <el-form :model="unitsQuery"
               ref="unitsQuery"
               :inline="true">
        <el-form-item label="编码"
                      prop="unitsNum">
          <el-input v-model="unitsQuery.unitsNum"
                    placeholder="请输入编码 "
                    clearable
                    size="small"
                    style="width: 120px"
                    @keyup.enter.native="handleQueryDialog" />
        </el-form-item>
        <el-form-item label="名称"
                      prop="unitsName">
          <el-input v-model="unitsQuery.unitsName"
                    placeholder="请输入名称 "
                    clearable
                    size="small"
                    style="width: 120px"
        ...
        <el-form-item label="单位类型"
                      prop="unitsType">
          <el-select v-model="unitsQuery.unitsType"
                     placeholder="选择单位类型"
                     clearable
                     size="small"
                     style="width: 150px">
        ...
        <el-form-item>
          <el-button type="primary"
                     icon="el-icon-search"
                     size="mini"
                     @click="handleQueryDialog">搜索</el-button>
        </el-form-item>
      </el-form>
      <el-table v-loading="unitsLoading"
                :data="unitsListData"
      ...
      <div slot="footer"
           class="dialog-footer">
        <el-button type="primary"
                   @click="submitFormDialog">确 定</el-button>
    ...
    <el-dialog :title="title"
      ...
      <el-table v-loading="imageLoading"
                :data="imageListData">
        ...
        <el-table-column label="操作"
          ...
          <template slot-scope="scope">
            <el-button size="mini"
                       type="text"
                       icon="el-icon-edit"
                       @click="handleUpdateFile(scope.row)">标注</el-button>
            <el-button size="mini"
                       type="text"
                       icon="el-icon-info"
                       @click="handlePreviewFile(scope.row)">查看</el-button>
            <el-button size="mini"
                       type="text"
                       icon="el-icon-upload"
                       style="color:#e6a23c"
                       @click="handleDownloadFile(scope.row)">下载</el-button>
            <el-button size="mini"
                       type="text"
                       icon="el-icon-delete"
                       style="color:#fd5656"
                       @click=" handleRemoveFile(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="pagination">
        <el-pagination background
                       ...
                       @current-change="handlePageChangeFileDialog"></el-pagination>
      </div>
      <el-dialog :title="imageName"
                 :visible.sync="filePreviewOpen"
                 append-to-body>
        <img :src="imageUrl"
             style="width:100%">
      </el-dialog>
      <el-dialog title="标注"
        ...
        <el-form :model="remarkForm"
          ...
          <el-form-item label=""
                        prop="remark">
            <el-input v-model="remarkForm.remark"
                      style="width:350px"
                      type="textarea" />
          </el-form-item>
        </el-form>
        <div slot="footer"
             class="dialog-footer">
          <el-button type="primary"
                     @click="submitRemarkForm">确 定</el-button>
        </div>
      </el-dialog>
    </el-dialog>
  </div>
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        materielNum: [{ required: true, message: '产品编码不能为空', trigger: 'blur' }, { max: 10, message: '长度不能超过10个字符', trigger: 'blur' }],
  ...
  methods: {
    ...
    handleDelete () {
      ...
      this.$confirm('请确认是否删除？', '警告', {
        ...
        return deleteClientele(row.materielId).then(res => that.callbackFun(res));
      })
    },
    /** 新增按钮操作 */
    handleAdd () {
    ...
    getMaterielFile () {
      listMaterielFile(this.fileQuery).then(res => {
        // console.log("listMaterielFile", res)
        this.imageListData = res.data.records;
        this.fileTotal = res.data.total;
      })
    },
    ...
    handleDownloadFile (row) {
      ...
      this.imageLoading = true
      downloadFile(row.pkId).then(res => {
        console.log("下载成功")
