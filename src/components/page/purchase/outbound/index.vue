<template>
    <div class="container">
        ...
        >
            ...
            <el-table-column label="出库仓库编码" align="center" prop="warehouseNum" :show-overflow-tooltip="true" width="150" />
            <el-table-column label="出库仓库名称" align="center" prop="warehouseName" :show-overflow-tooltip="true" width="150" />
            <el-table-column label="出库日期" align="center" prop="outboundTime" :show-overflow-tooltip="true" width="150" />
            <el-table-column label="出库状态" prop="outboundStatus" :formatter="outboundStatusFormatter" align="center" width="100" />
            ...
            <el-table-column label="操作" align="center" width="100" class-name="small-padding fixed-width" fixed="right">
                <template slot-scope="scope">
                    <el-button size="mini" type="text" icon="el-icon-edit" v-hasPermi="['purchase:returns:edit']" @click="handleUpdate(scope.row)">出库</el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
import { listPurchaseOutbound } from '@/api/purchase/returns.js';
export default {
    data() {
        return {
            // 遮罩层
    ...
    methods: {
        ...
        getList() {
            listPurchaseOutbound(this.queryParams).then(res => {
                if (res.success) {
        ...
        handleUpdate(row) {
            ...
            this.$router.push({ path: '/page/purchase/outbound/edit', query: { id: row.returnsId } });
        },
        /** 明细按钮操作 */
        handlePreview(row) {
            this.$router.push({ path: '/page/purchase/outbound/preview', query: { id: row.returnsId, isShow: false } });
        },
        handleAudit() {
            if (this.selection.outboundStatus === 0) {
                this.msgError('请先出库');
                return;
            }
            this.$router.push({ path: '/page/purchase/outbound/preview', query: { id: this.selection.returnsId, isShow: false, isAudit: true } });
        }
    }
};
</script>
