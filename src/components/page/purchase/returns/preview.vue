...
<script>
import { auditPurchaseReturns, getPurchaseReturns, listPurchaseReturnsSub } from '@/api/purchase/returns.js';
...
export default {
    ...
    watch: {
        $route(to, form) {
            if (to.path === '/page/purchase/returns/preview' && this.supplierForm.returnsId !== this.$route.query.id) {
                this.getPurchaseReturnsData();
            }
        }
    },
    methods: {
        ...
        getPurchaseReturnsData() {
            ...
            getPurchaseReturns(id).then(res => {
