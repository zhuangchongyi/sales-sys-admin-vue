<template>
    <div class="container">
        <div>
            <el-button type="primary" icon="el-icon-check" @click="submitAddForm">签 收</el-button>
            <!-- <el-button icon="el-icon-close" @click="clearAddForm">取 消</el-button> -->
        </div>
            ...
            <el-row>
                <el-col :span="4">
                    <el-form-item label="签收日期" prop="signbackTime">
                        <el-date-picker type="date" v-model="clienteleForm.signbackTime" placeholder="选择日期" size="small" style="width: 150px;" value-format="yyyy-MM-dd" format="yyyy-MM-dd" />
                    </el-form-item>
                </el-col>
                <el-col :span="4">
        ...
        <el-table v-loading="loading" :data="materielList">
            ...
            <el-table-column label="签收数" prop="signNum" width="120" align="center">
                <template slot-scope="scope">
                    <el-input size="mini" oninput="value=value.replace(/[^\d]/g,'')" maxLength="9" @input="calculateTotal(scope.row)" v-model="scope.row.signNum" />
                </template>
            </el-table-column>
...
export default {
    ...
    watch: {
        $route(to, form) {
            if (to.path === '/page/sales/signBack/edit' && this.signbackId !== this.$route.query.signId) {
